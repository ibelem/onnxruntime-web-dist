...
`);let T="";c<2&&f>0?T="coords":T=s.map((G,K)=>`coords.${v[K+b]}`).join(", ");let w="return outputValue;",$=ee.size(s)===1,C=ee.size(a)===1;if(f===1&&!$&&!C)w=`
    ...
    `;return new Z(z,["coordinates.getOutputCoords"])}getUnpackedSamplerAtOutputCoords(n,t,o,i){let s=[o.width,o.height],a=[t.width,t.height],u=t.unpackedShape.length,l=o.unpackedShape.length,f=t.unpackedShape,c=o.unpackedShape,p=zo(i);if(u===l&&Rr.arraysEqual(a,s)){let $=`
          float ${n}() {
...
`)};
    ...
    ${p}`},u=[{type:12,data:s},...H(e,t)];return{name:"Expand",shaderCache:{hint:`${t.length}`,inputDependencies:["rank"]},getShaderSource:a,getRunData:()=>({outputs:[{dims:t,dataType:r[0].dataType}],dispatchGroup:{x:Math.ceil(s/64)},programUniforms:u})}},uy=r=>{R1(r.inputs),r.compute(M1(r.inputs),{inputs:[0]})}});var V1,cy,fy=E(()=>{"use strict";fe();ge();ye();zi();V1=r=>{let e=r[0].dataType,n=k.size(r[0].dims),t=k.size(r[1].dims),o=t%4===0,i=s=>{let a=B("x",e,[1],4),u=B("bias",e,[1],4),l=F("y",e,[1],4),f=[{name:"output_vec_size",type:"u32"},{name:"bias_size",type:"u32"}],c=g=>`
